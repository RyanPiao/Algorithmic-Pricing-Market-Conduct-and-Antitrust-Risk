...
**City-specific estimates.** @tbl-itt-city reports city-specific estimates at the ±60-day bandwidth on residualized outcomes. All eight cities show positive point estimates, ranging from Los Angeles ($+$0.06%) to Chicago ($+$5.6%). The universally positive sign is informative: under the null hypothesis of zero effect, the probability of all 8 cities showing positive estimates is $(1/2)^8 = 0.4\%$. A formal sign test rejects the null at the 1% level ($p = 0.004$, one-sided). A city-level t-test treating the 8 estimates as independent observations yields a mean effect of 1.84% with $t = 3.04$ ($p = 0.019$, two-sided; 95% CI: [0.41%, 3.27%]). A Wilcoxon signed-rank test confirms ($p = 0.008$).

{{< include ../output/tables/nonparametric_tests.md >}}

: Nonparametric tests on city-level estimates {#tbl-nonparam}

These nonparametric tests complement the bootstrap inference. The bootstrap (p = 0.15-0.20) is sensitive to the variance across cities — Chicago's large 5.6% effect inflates the standard deviation of the city-level estimates, reducing precision. The sign and rank tests are distribution-free and do not depend on magnitudes, only on the consistency of the sign. **The evidence suggests the effect is probably positive, but the common-cutoff design lacks the precision to reliably quantify its magnitude.**

{{< include ../output/tables/itt_rdd_city.md >}}

: City-specific ITT RDD estimates {#tbl-itt-city}

**City heterogeneity.** The cross-city variation in point estimates is informative. Chicago (+5.6%) and Seattle (+2.9%) --- mid-size markets with moderate regulation --- show the largest effects. Los Angeles (+0.1%) and San Francisco (+0.9%) --- large, heavily regulated markets --- show the smallest. This pattern is consistent with regulatory stringency constraining algorithmic pricing effects: in markets where hosts face binding STR regulations (registration requirements, night caps, zoning restrictions), the algorithm's ability to adjust prices may be limited by the regulatory environment. However, the pattern could also reflect differences in market thickness, tourist demand seasonality, or host professionalization. With only eight cities, we cannot identify the specific moderator.
